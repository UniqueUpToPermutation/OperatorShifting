...
void VectorDistributionFromLambda::drawSample(arma::vec *v) const {
    lambda_func(v);
}

VectorDistributionFromLambda::VectorDistributionFromLambda(std::function<void(arma::vec *)>& lambda_func) {
...
void IdenticalVectorDistributionFromLambda::drawSample(arma::vec *v1, arma::vec *v2) const {
    lambda_func(v1);
    *v2 = *v1;
}
...
void DefaultSparseMatrixSample::solve(const arma::vec &b, arma::vec* result) const {
    *result = arma::spsolve(sparse_mat, b);
}

void DefaultSparseMatrixSample::apply(const arma::vec &b, arma::vec *result) const {
    *result = sparse_mat * b;
}

DefaultSparseMatrixSample::DefaultSparseMatrixSample(arma::sp_mat &sparse_mat) {
...
void IdentityMatrixSample::solve(const arma::vec &b, arma::vec *result) const {
    *result = b;
}

void IdentityMatrixSample::apply(const arma::vec &b, arma::vec *result) const {
    *result = b;
}
              ...
              const IMatrixOperator* op_B)
{
    ...
    arma::vec q;
    arma::vec u;
    arma::vec a_boot_inv_q;
    arma::vec a_inv_q;
    arma::vec w_a_boot_inv_q;
    arma::vec temp;
    arma::vec temp2;
    arma::vec a_boot_inv_u;
    arma::vec wb_a_boot_inv_u;
    ...
    for (int i_system = 0; i_system < num_system_samples; ++i_system)
    {
        ...
        for (int i_rhs = 0; i_rhs < num_per_system_samples; ++i_rhs)
        {
            ...
            if (!op_Mhat_bootstrap->isIdentity()) {
                temp = q;
            ...
            numerator += arma::dot(w_a_boot_inv_q, a_boot_inv_q - a_inv_q);
            ...
            denominator += arma::dot(wb_a_boot_inv_u, a_boot_inv_u);
        }
    }
                ...
                const IVectorDistribution* q_dist)
{
    ...
    arma::vec q;
    arma::vec Ahat_boot_inv_q;
    arma::vec Ahat_Ahat_boot_inv_q;
    arma::vec temp;
    ...
    for (int i_system = 0; i_system < num_system_samples; ++i_system)
    {
        ...
        for (int i_rhs = 0; i_rhs < num_per_system_samples; ++i_rhs)
        {
            ...
            double term1 = arma::dot(Ahat_boot_inv_q, Ahat_Ahat_boot_inv_q);
            double term2 = arma::dot(q, Ahat_boot_inv_q);
    ...
    for (int i_system = 0; i_system < num_system_samples; ++i_system)
    {
        ...
        for (int i_rhs = 0; i_rhs < num_per_system_samples; ++i_rhs) {
            ...
            for (int i = 0; i < order + 1; ++i) {
                double dot_prod = arma::dot(Ahat_inv_q, pows_q[i]);
...
{
    arma::vec v_last = arma::randn(dimension);
    arma::vec a_inv_v_last;
    arma::vec v;
    arma::vec a_inv_v;
    ...
    double last_eig = arma::dot(a_inv_v, v) / arma::dot(a_inv_v_last, v_last);
    double eig = 0.0;
    ...
    while (true) {
        ...
        eig = arma::dot(a_inv_v, v) / arma::dot(a_inv_v_last, v_last);
