...
void dgnGridLaplacian1D(int argc, char** argv) {
    ...
    run = std::shared_ptr<ProblemRunType>(
            new OpshiftRun<GridLaplacian1DParameters, GridLaplacian1DHyperparameters>(problem_def.get()));
    run->numberSubRuns = numSubRuns;
    run->samplesPerSubRun = samplesPerSubRun;
    diagnostics.addRun(run);

    run = std::shared_ptr<ProblemRunType>(
            new EnergyOpshiftRun<GridLaplacian1DParameters, GridLaplacian1DHyperparameters>(problem_def.get()));
    run->numberSubRuns = numSubRuns;
    run->samplesPerSubRun = samplesPerSubRun;
    diagnostics.addRun(run);

    run = std::shared_ptr<ProblemRunType>(
            new EnergyOpshiftTruncatedRun<GridLaplacian1DParameters,
    ...
    run = std::shared_ptr<ProblemRunType>(
            new EnergyOpshiftTruncatedRebasedAccelRun<GridLaplacian1DParameters,
                    GridLaplacian1DHyperparameters>(problem_def.get(), 2));
    run->numberSubRuns = numSubRuns;
    run->samplesPerSubRun = samplesPerSubRun;
    diagnostics.addRun(run);
