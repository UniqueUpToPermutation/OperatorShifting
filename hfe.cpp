...
namespace hfe {
		...
		if (hasPositions) {
			...
			for (int i = 0, j = 0; i < vertexCount; ++i) {
				geo->vertexPositions.emplace_back(
					Eigen::Vector3d{ floatBuf[j++], floatBuf[j++], floatBuf[j++] }
				);
			}
		}
		if (hasUvs) {
			...
			for (int i = 0, j = 0; i < vertexCount; ++i) {
				geo->vertexUVs.emplace_back(
					Eigen::Vector2d{ floatBuf[j++], floatBuf[j++] }
				);
			}
		}
		if (hasNormals) {
	...
	{
		Eigen::Vector3d lower = Eigen::Vector3d{ std::numeric_limits<float>::infinity(), 
			std::numeric_limits<float>::infinity(), 
			std::numeric_limits<float>::infinity() };
		Eigen::Vector3d upper = Eigen::Vector3d{ -std::numeric_limits<float>::infinity(), 
			-std::numeric_limits<float>::infinity(), 
			-std::numeric_limits<float>::infinity() };
	...
	Geometry* loadJson(const std::string& filename) {
		...
		if (hasPositions) {
			...
			for (int i = 0; i < vertexCount * 3; i += 3)
				geo->vertexPositions.emplace_back(Eigen::Vector3d{ vertexPositions[i], vertexPositions[i + 1], vertexPositions[i + 2] });
		}
		if (hasUvs) {
			geo->vertexUVs.reserve(vertexCount);
			for (int i = 0; i < vertexCount * 2; i += 2)
				geo->vertexUVs.emplace_back(Eigen::Vector2d{ vertexUvs[i], vertexUvs[i + 1] });
		}
		if (hasNormals) {
			geo->vertexNormals.reserve(vertexCount);
			for (int i = 0; i < vertexCount * 3; i += 3)
				geo->vertexNormals.emplace_back(Eigen::Vector3d{ vertexNormals[i], vertexNormals[i + 1], vertexNormals[i + 2] });
		}
		if (hasTangents) {
			geo->vertexTangents.reserve(vertexCount);
			for (int i = 0; i < vertexCount * 3; i += 3)
				geo->vertexTangents.emplace_back(Eigen::Vector3d{ vertexTangents[i], vertexTangents[i + 1], vertexTangents[i + 2] });
		}			geo->vertices.reserve(vertexCount);
    ...
    void Geometry::copyTo(Geometry* output) const {
    ...
    Geometry* Geometry::deepCopy() const {
